...
namespace hnswlib
{
    ...
    public:
        ...
        virtual bool operator()(hnswlib::labeltype id, size_t q_id) { return true; }
    ...
    public:
        ...
        // virtual std::priority_queue<std::pair<dist_t, labeltype>>
        // searchKnnWithFilter(const void *, size_t, size_t, BaseFilterFunctor *isIdAllowed) const = 0;

        // virtual std::priority_queue<std::pair<dist_t, labeltype>>
        // searchKnnWithoutFilter(const void *, size_t) const = 0;

        // Return k nearest neighbor in the order of closer fist
        // virtual std::vector<std::pair<dist_t, labeltype>>
        // searchKnnCloserFirst(const void *query_data, size_t k, size_t q_id, BaseFilterFunctor *isIdAllowed = nullptr) const;
    ...
    // AlgorithmInterface<dist_t>::searchKnnCloserFirst(const void *query_data, size_t k, size_t q_id,
    //                                                  BaseFilterFunctor *isIdAllowed) const
    // {
    //     std::vector<std::pair<dist_t, labeltype>> result;

    //     // here searchKnn returns the result in the order of further first
    //     auto ret = searchKnn(query_data, k, q_id, isIdAllowed);
    //     {
    //         size_t sz = ret.size();
    //         result.resize(sz);
    //         while (!ret.empty())
